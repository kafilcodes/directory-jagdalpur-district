...
export default async function DashboardPage() {
  ...
  if (!user) {
    // Auth-protected: redirect home if not signed in
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Provider Dashboard</h1>
        <AuthButtons />
      </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-gray-600">Recent Events</CardTitle>
          </CardHeader>
        ...
        {Object.entries(counts).slice(0, 2).map(([k, v]) => (
          <Card key={k}>
            <CardHeader className="pb-2">
              <CardTitle className="text-base capitalize text-gray-600">{k}</CardTitle>
            </CardHeader>
